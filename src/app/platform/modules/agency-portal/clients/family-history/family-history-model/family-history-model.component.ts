...
import { PatientMedicalFamilyHistoryModel, PatientMedicalFamilyHistoryDiseaseModel } from '../family-history.model';
...
export class FamilyHistoryModelComponent implements OnInit {
  ...
  masterStatus: Array<any> = [{ id: true, value: "Active" }, { id: false, value: "In Active" }]
  maxDate = new Date();
  ...
  initializeFormFields(familyHistoryObj?: PatientMedicalFamilyHistoryModel) {
    familyHistoryObj = familyHistoryObj || new PatientMedicalFamilyHistoryModel();
    ...
    if (familyHistoryObj.patientMedicalFamilyHistoryDiseases && familyHistoryObj.patientMedicalFamilyHistoryDiseases.length) {
  ...
  get patientMedicalFamilyHistoryDiseases() {
    return this.familyHistoryForm.get('patientMedicalFamilyHistoryDiseases') as FormArray;
  }
  addDiseaseFields(diseaseObj?: PatientMedicalFamilyHistoryDiseaseModel) {
    const diseaseControls = diseaseObj || new PatientMedicalFamilyHistoryDiseaseModel();
    this.patientMedicalFamilyHistoryDiseases.push(this.formBuilder.group(diseaseControls));
  }
  removeDiseaseFields(ix: number) {
    this.patientMedicalFamilyHistoryDiseases.removeAt(ix);
  }
  get formControls() { return this.familyHistoryForm.controls; }
  onSubmit() {
    ...
    postData = {
      ...
      linkedEncounterId: this.linkedEncounterId,
      patientMedicalFamilyHistoryDiseases: (this.familyHistoryForm.value.patientMedicalFamilyHistoryDiseases || []).map((obj) => {
        return {
          ...obj,
    ...
    this.familyHistoryService.savePatientFamilyHistoryData(postData).subscribe((response: any) => {
  ...
  formControlValueChanged() {
    ...
    this.familyHistoryForm.get('dateOfDeath').valueChanges.subscribe(
      (mode: any) => {
        if (mode != null) {
