...
this.commonService.loginUser.subscribe((user: LoginUser) => {
  if (user.data) {
    ...
    this.userId = user.data.userID;
      ...
      if (this.patientAppointmentId > 0 && Number(this.userId) > 0) {
      console.log("inside publisher method")
      this.appService
        .getEndCall(this.patientAppointmentId, Number(this.userId))
        .subscribe((res) => { 
        });
      }
  }
  ...
  startVideoCall() {
    ...
    // this.hubService.createHubConnection(this.userId);
