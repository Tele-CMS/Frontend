...
export class ProgramsComponent implements OnInit {
  ...
  constructor(
    ...
    this.displayedColumns = [
      { displayName: 'Program Name', key: 'diseaseManageProgram', class: '', width: '20%',isSort:true },
      { displayName: 'Graduation Date', key: 'graduationDate', class: '', width: '10%', type: "date",isSort:true },
      { displayName: 'Enrollment Date', key: 'dateOfEnrollment', class: '', width: '15%', type: "date" ,isSort:true},
      { displayName: 'Termination Date', key: 'dateOfTermination', class: '', width: '15%', type: "date",isSort:true },
      { displayName: 'Provider', key: 'careManager', class: '', width: '15%',isSort:true },
      { displayName: 'Status', key: 'status', class: '', width: '15%',isSort:true },
      { displayName: 'Frequency', key: 'frequency', class: '', width: '15%',isSort:true },
  ...
  getPatientDiseaseManagementProgramList() {
    ...
    this.clientProgramsService.getPatientDiseaseManagementProgramList(filterParams).subscribe((response: any) => {
  ...
  onTableActionClick(actionObj?: any) {
    ...
    switch ((actionObj.action || '').toUpperCase()) {
      case 'VIEW':
        ...
        break;
      case 'EDIT':
        this.clientProgramsService.getDiseaseManagementProgramDetails(actionObj.data.patientDiseaseManagementProgramId).subscribe((response: any) => {
          if (response.statusCode == 200) {
        ...
        break;
      case 'DELETE':
        this.clientProgramsService.deleteDiseaseManagementProgram(actionObj.data.patientDiseaseManagementProgramId).subscribe((response: any) => {
          if (response.statusCode == 200) {
        ...
        break;
      case 'TERMINATE':
        this.clientProgramsService.terminateActivity(actionObj.data.patientDiseaseManagementProgramId, new Date(), false).subscribe((response: any) => {
          if (response.statusCode == 200) {
            this.notifier.notify('success', response.message)
            this.getPatientDiseaseManagementProgramList();
          } else {
  ...
  getUserPermissions() {
    ...
    const actionPermissions = this.clientProgramsService.getUserScreenActionPermissions('CLIENT', 'CLIENT_PROGRAMS_LIST');
