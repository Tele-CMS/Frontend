...
export class VitalsListComponent implements OnInit { 
  ...
  //constructor
  constructor(private activatedRoute: ActivatedRoute,  private notifier: NotifierService, private clientsService: ClientsService, public activityModal: MatDialog, private dialogService: DialogService, private commonService: CommonService,private schedulerService: SchedulerService) {
      ...
      (user: LoginUser) => { 
          ...
          if (user.data) {
              ...
              if ((userRoleName || "").toUpperCase() === "CLIENT") {
                  this.isPatient = true;
                  this.clientId = user.patientData.patientId;
                  this.getVitalList(this.filterModel,'grid');
                  this.actionButtons = [];
              }
              else {
                console.log("vitals else...");
                
                this.commonService.appointmentData.subscribe((response:any) =>{ 
                  this.schedulerService.getAppointmentDetailsWithPatient(response).subscribe((response: any) => {
