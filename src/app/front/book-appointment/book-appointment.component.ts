...
import { getMatIconFailedToSanitizeLiteralError, MatAutocomplete, MatAutocompleteSelectedEvent, MatChipInputEvent, MatDialog, MatDialogRef, MAT_DIALOG_DATA } from "@angular/material";
import {
  Component,
  OnInit,
  ViewEncapsulation,
...
export class BookAppointmentComponent implements OnInit {
              ...
              if (slots.length > 0) {
                slots.forEach(x => {
                  this.providerAvailiabilitySlots.push({
                    startTime: x.startTime ,//.replace(":00","").replace("AM","").replace("PM",""),
                    endTime: x.endTime,//.replace(":00",""),
  ...
  onTypeChange(type: any) {
    ...
    if (type.toLowerCase() == 'followup') {
      ...
      this.schedulerService.GetLastPatientFollowupDetails(this.patient).subscribe(res => {
    ...
    this.schedulerService
      ...
      .subscribe(response => {
        ...
        if (response.statusCode === 200) {
          ...
          if(localStorage.getItem("reportObject")!=null && localStorage.getItem("reportObject")!=undefined)
          {
          ...
          if(object!=null && object !=undefined)
          {
            ...
            this.schedulerService.SaveSymptomatePatientReport(JSON.stringify(object)).subscribe((result: any) => {
  ...
  termsandconditionchecked(event) {
    if (event.checked) {
  ...
  handleImageChange(e) {
    ...
    var validSize=this.commonService.calculateFileSizehandler(files);
    if(!validSize)
    {
      this.notifier.notify("error","Please select file size less than 5 MB ");
      return null;
    }
